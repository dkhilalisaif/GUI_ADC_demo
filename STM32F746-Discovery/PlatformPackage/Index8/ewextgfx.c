...
/* The color format of the framebuffer has to correspond to color format of the
  ...
  #error The given EW_FRAME_BUFFER_COLOR_FORMAT is not supported! Use Index8 within your makefile!
#endif
...
*
*******************************************************************************/
int GfxLoadClut( unsigned long* aColors, int aIndex, int aCount )
{
  /* copy CLUT content and reorder color components */
  for ( ; aCount > 0; aCount--, aIndex++, aColors++ )
  {
    register unsigned long c = *aColors;
    Clut[ aIndex ] = ( c & 0xFF00FF00 ) | (( c & 0x00FF0000 ) >> 16 ) | (( c & 0x000000FF ) << 16 );
  }
...
void GfxUseGraphicsAccelerator( int aActive )
{
  ...
  EwPrint( "Graphics Accelerator is not used for Index8 color format.\n" );
}
