...
#if ( EW_FRAME_BUFFER_COLOR_FORMAT != EW_FRAME_BUFFER_COLOR_FORMAT_RGBA4444 )
  #error The given EW_FRAME_BUFFER_COLOR_FORMAT is not supported! Use RGBA4444 within your makefile!
#endif
...
/* define size of a single cache line and macros to invalidate or clean the data cache */
  ...
  #define EW_INVALIDATE_CACHE( addr, size ) SCB_InvalidateDCache_by_Addr((uint32_t*)(addr), (int32_t)(size))
  #define EW_CLEAN_CACHE( addr, size )      SCB_CleanDCache_by_Addr((uint32_t*)(addr), (int32_t)(size))
  ...
  void* aDisplay1, void* aDisplay2, void* aDisplay3 )
{
  ...
  viewport->FrameBuffer->OrigPixel      = aDisplay1;
    ...
    if ( !EwBspGraphicsInit( DMA2D_OUTPUT_ARGB4444 ))
    {
      EW_ERROR( Err05 );
      return 0;
    }
...
/* helper function to convert the CLUT content in a format suitable for hardware accelerator */
void STM32ConvertClut( unsigned long* aDst, unsigned long* aSrc )
{
  int i;
  if ( aDst && aSrc )
    for ( i = 0; i < 256; i++, aDst++, aSrc++ )
  {
    uint32_t color = (( *aSrc & 0xF000 ) << 12 )
                   | (( *aSrc & 0x0F00 ) << 8 )
                   | (( *aSrc & 0x00F0 ) << 4 )
                   | (( *aSrc & 0x000F ));
    *aDst = color | ( color << 4 );
...
unsigned long GfxCreateSurface( int aFormat, int aWidth, int aHeight )
{
  ...
  else if ( aFormat == EW_PIXEL_FORMAT_INDEX8 )
  {
    ...
    #ifdef EW_USE_GRAPHICS_ACCELERATOR
      accelClutSize = 256 * 4 + 3; /* clut startaddress has to be DWORD aligned */
    #endif
  }
  else
    return 0;
  ...
  surface = (XGfxSurface*)EwAllocVideo( sizeof( XGfxSurface ) + bitmapSize + clutSize + accelClutSize );
  if ( !surface )
    return 0;
  ...
  surface->AllocSize      = bitmapSize + clutSize + accelClutSize;
  ...
  if ( clutSize > 0 )
    ...
    if ( accelClutSize > 0 )
      surface->AccelClut  = (void*)(((unsigned long)( surface + 1 ) + bitmapSize + clutSize + 3 ) &~ 3 );
  ...
  XSurfaceMemory* aMemory )
{
  ...
  surface = (XGfxSurface*)EwAllocVideo( sizeof( XGfxSurface ) + accelClutSize );
  if ( !surface )
    return 0;
  EwZero( surface, sizeof( XGfxSurface ));
  ...
  surface->BytesPerPixel  = bytesPerPixel;
  surface->Format         = aFormat;
  surface->AllocSize      = accelClutSize;
  ...
  surface->Clut           = (void*)( aMemory->Clut );
   ...
   if ( accelClutSize > 0 )
   {
      surface->AccelClut   = (void*)(((unsigned long)( surface + 1 ) + 3 ) &~ 3 );

      /* convert the CLUT content in a format suitable for hardware accelerator */
      STM32ConvertClut( (unsigned long*)surface->AccelClut, (unsigned long*)surface->Clut );
   }
  ...
  int aWriteClut )
{
  ...
  EW_UNUSED_ARG( aWritePixel );
    ...
    if ( aWriteClut )
    {
      XGfxSurface* surface = (XGfxSurface*)aSurfaceHandle;
...
#endif
  {
    uint32_t dstColorFormat = DMA2D_OUTPUT_ARGB4444;
  ...
  int aBlend, unsigned long* aColors )
{
  ...
  {
    uint32_t dstColorFormat = DMA2D_OUTPUT_ARGB4444;
    uint32_t srcColorFormat = DMA2D_INPUT_ARGB4444;
    uint32_t color          = ( EW_ALPHA( aColors[0]) << 24 )
                            | ( EW_RED  ( aColors[0]) << 16 )
                            | ( EW_GREEN( aColors[0]) <<  8 )
                            |   EW_BLUE ( aColors[0]);
    ...
    else if ( srcFormat == EW_PIXEL_FORMAT_INDEX8 )
    {
      ...
      if ( srcSurface != LoadedClutSurface )
      {
        ...
        EwBspGraphicsLoadClut( (uint32_t)srcSurface->AccelClut, 256 );
